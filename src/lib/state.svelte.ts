...
import type { Layer, ExportFormat, ProgressInfo } from '$lib/schwarzplan/types';
...
class AppState {
    // --- Map State ---
    location = $state({
        center: [13.4050, 52.5200] as [number, number],
        zoom: 13
    });
    ...
    fitBounds(minLon: number, minLat: number, maxLon: number, maxLat: number) {
        this.requestExtent = [[minLon, minLat], [maxLon, maxLat]];
        this.requestFlyTo = null;
    }
    ...
    private saveToCookie(name: string, value: any) {
        if (!browser) return;
