...
async function getOSMdata(overpassApi, latA, lonA, latB, lonB, dataType, allWorkers, _callback){
    ...
    console.log(result);
    _callback(JSON.parse(result), dataType, allWorkers);
}
...
onmessage = function(e) {
    ...
    var [thisID,latA,lonA,latB,lonB,mlatA,mlonA,mlatB,mlonB,heightMeters,widthMeters,overpassApi,dataArray] = e.data

    allWorkers = [];
    resultArray = [];

    dataArray.forEach(function(dataType){

        getOSMdata(overpassApi,latA,lonA,latB,lonB,dataType,allWorkers, function(osm, dataType, allWorkers){
            ...
            for (var i=0; i < mgjson.features.length; i += oneStep){
                ...
                worker.postMessage([newDict, mlonA, mlatA, mlonB, mlatB, widthMeters, heightMeters, thisID, dataType])
